...
export function VideoContent({ enrollment }) {
    ...
    const saveCurrentPosition = (moduleIndex, lessonIndex) => {
    ...
    const saveAssignmentSubmissions = (submissions) => {
        if (!user?.id || !courseId) return
    ...
    const saveSubmittedAssignments = (submittedSet) => {
        if (!user?.id || !courseId) return
    ...
    const loadSubmittedAssignments = () => {
    ...
    const saveVideoProgress = (progressData) => {
        if (!user?.id || !courseId) return
